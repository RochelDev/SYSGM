...
@section('content')
    ...
    <div class="flex justify-between items-center mb-3">
        <h1 class="text-2xl font-bold tracking-tight">Liste des Postes</h1>
    ...
    <div class="mb-4">
        <label for="structure_filter" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Filtrer par Structure :</label>
        <select id="structure_filter" name="structure_filter" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 dark:bg-dark-700 dark:text-white">
            <option value="">-- Tous les Postes --</option>
            @foreach ($structures as $structure)
                <option value="{{ $structure->id }}" @if(request('structure_filter') == $structure->id) selected @endif>{{ $structure->nom_structure }}</option>
            @endforeach
        </select>
    </div>
    ...
    <script>
        document.getElementById('structure_filter').addEventListener('change', function() {
            window.location.href = '{{ route('admin.poste.index') }}?structure_filter=' + this.value;
        });
    </script>
@endsection